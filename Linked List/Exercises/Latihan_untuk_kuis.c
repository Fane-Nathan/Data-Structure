...
char *rubiksType() {
  ...
  do {
    printf("Please input rubik's type [Magnetic / Non-Magnetic] [Case "
           "sensitive]: ");
    fgets(rubiksType, 20, stdin);
    rubiksType[strcspn(rubiksType, "\n")] = '\0';
    if (strcmp(rubiksType, "Magnetic") != 0 &&
        strcmp(rubiksType, "Non-Magnetic")) {
      printf(
          "\nInvalid type. Type must be either Magnetic or Non-Magnetic\n\n");
    }
  } while (strcmp(rubiksType, "Magnetic") != 0 &&
           strcmp(rubiksType, "Non-Magnetic") != 0);
  return rubiksType;
}
...
Node *createNode(char *Name, char *rubiksType, char *rubiksID,
                 int rubiksQuantity, int rubiksPrice) {
...
void inputData(Node **head, Node **tail) {
  ...
  int price =
      (strcmp(type, "Magnetic") == 0) ? rubiksPrice() : rubiksPriceNonMagnet();
