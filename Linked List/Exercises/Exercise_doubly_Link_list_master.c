...
char *price() {
  ...
  do {
    ...
    if (price >= 1000000000) {
      sprintf(priceStr, "Rp. %d.%03d.%03d.%03d", price / 1000000000,
              (price / 1000000) % 1000, (price / 1000) % 1000, price % 1000);
    } else if (price >= 1000000) {
      sprintf(priceStr, "Rp. %d.%03d.%03d", price / 1000000,
              (price / 1000) % 1000, price % 1000);
    } else {
...
void addCake(Node **tail, Node **head) {
  ...
  (insertCake(head, tail, newCake) == 1)
      ? printf("The cake has been added\n")
  ...
  if (temp != NULL) {
    ...
    if (temp == NULL) {
      ...
      printf("%-7s | %-10s| %-10s| %-10s| %-10s\n", "No.", "Cake Code",
             "Cake Name", "Available", "Price");
      printf("-----------------------------------------------------------------"
             "------------------\n");
      printf("%-7s| %-10s| %-10s| %-10d| %-10s\n", "1", temp->cakeID,
             temp->cake, temp->sum, temp->price);
      printf("-----------------------------------------------------------------"
             "------------------\n");
    }
  }
  ...
  if (currentNode != NULL) {
    while (currentNode != NULL) {
      printf("%-7d| %-10s| %-10s| %-10d| %-10s\n", counter++,
             currentNode->cakeID, currentNode->cake, currentNode->sum,
             currentNode->price);
      printf("-----------------------------------------------------------------"
             "------------------\n");
