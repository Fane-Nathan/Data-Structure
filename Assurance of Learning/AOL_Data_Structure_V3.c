...
void insertAtBeginning(DataType dataType, char *words, int number,
                       Node **head) {
...
void insertNode(DataType dataType, char *words, int number, Node **head,
                int position) {
...
void delete(DataType dataType, int number, char *words, Node **head) {
  ...
  while (temp != NULL) {
    if (temp->dataType == dataType) {
      if ((dataType == INT && temp->data.number) ||
          (dataType == STRING && strcmp(temp->data.words, words) == 0)) {
    ...
    if (current->dataType == dataType) {
      if (dataType == INT && current->data.number == number) {
        return position;
                 ...
                 strcmp(current->data.words, words) == 0) {
        return position;
      }
    }
...
int main(void) {
  ...
  printf("\nThis is a program to store a data from a user either integer or "
         "word.\n");
  do {
    printf("\n==============================\n");
